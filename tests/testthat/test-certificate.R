## Section 6. build_certificate() and cert_rows() are internal: a certificate is
## something a caller reads, never something a caller builds. These assert the
## contract every shape in the package depends on -- the roll-up rule, what the
## vocabulary refuses, and that a row's evidence survives into evidence_satisfies().
...
test_that("overall is a roll-up of the four statuses and nothing else", {
  mk <- function(...) {
    ...
    linop:::build_certificate(r$collect(), subject = "test")
  }
...
test_that("without_deterministic_bound names the rows with no bound under them", {
  ...
  r$add("probabilistic", "pass", source = "probe", guarantee = "probabilistic_bound",
        confidence = 0.99)
  cert <- linop:::build_certificate(r$collect(), subject = "test")
...
test_that("the status vocabulary is closed, at both ends", {
  ...
  ## and again where a frame arrives from somewhere other than linop:::cert_rows()
  df <- data.frame(check = "x", status = "warn", source = "computation",
                   guarantee = "identity", confidence = 1, detail = "",
                   stringsAsFactors = FALSE)
  expect_error(linop:::build_certificate(df, subject = "test"), "unknown status")
  expect_error(linop:::build_certificate(df, subject = "test"), "counted as a pass")
})
...
test_that("build_certificate refuses a frame that is not a certificate table", {
  ...
  r$add("x", "pass")
  df <- r$collect()
  expect_error(linop:::build_certificate(df[, setdiff(names(df), "detail")], subject = "t"),
               "missing the column detail")
  expect_error(linop:::build_certificate(df[, c("check", "status")], subject = "t"),
               "missing the columns")
  expect_error(linop:::build_certificate(df[0, ], subject = "t"), "at least one row")
  expect_error(linop:::build_certificate(list(check = "x"), subject = "t"), "data frame")
  expect_error(linop:::build_certificate(df, subject = c("a", "b")), "single string")
})
...
test_that("a row's evidence fills the flat fields and survives into the object", {
  ...
  cert <- linop:::build_certificate(df, subject = "test", evidence = r$collect_evidence())
  expect_s3_class(cert$evidence[["forward error"]], "linop_evidence")

  ## and the laundering case reaches a certificate row the way it reaches a
...
test_that("a shape of its own prints and rolls up like every other", {
  ## The finite-section certificate of S0.6, hand-built here until the section node
  ## lands. Its rows are not the other shapes' rows: no residual line and no
  ## backward-error line, because a finite-section eigenvalue is not the answer to
  ## an equation with a right-hand side. The object around them is unchanged.
  eta <- 3.863955e-05
  gap <- 0.236067977
  ...
  cert <- linop:::build_certificate(r$collect(), subject = "finite section",
