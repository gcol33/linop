...
md <- c(
  ...
  section(
    ...
    paste(
      ...
      "operator supplies rather than a ranking: `convdiff_1d` offers the forward",
      "action alone, so only the two methods that need no adjoint appear on it,",
      "and CG is absent wherever the operator is indefinite. The `norm2()` row",
